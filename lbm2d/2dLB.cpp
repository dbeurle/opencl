...
inline int constexpr I2D(int ni, int i, int j) { return ni * j + i; }
...
int main(int argc, char** argv)
{
    ...
    tmpf0 = new float[ni * nj];
    tmpf1 = new float[ni * nj];
    tmpf2 = new float[ni * nj];
    tmpf3 = new float[ni * nj];
    tmpf4 = new float[ni * nj];
    tmpf5 = new float[ni * nj];
    tmpf6 = new float[ni * nj];
    tmpf7 = new float[ni * nj];
    tmpf8 = new float[ni * nj];

    plotvar = new float[ni * nj];
...
inline void stream(void)
{
    ...
    for (int j = 0; j < nj; j++)
    {
        auto const jm1 = (j == 0) ? 0 : j - 1;
        auto const jp1 = (j == nj - 1) ? nj - 1 : j + 1;

        for (int i = 1; i < ni; i++)
        {
            auto const i0 = I2D(ni, i, j);
            auto const im1 = (i == 0) ? 0 : i - 1;
            auto const ip1 = (i == ni - 1) ? ni - 1 : i + 1;
...
inline void collide()
{
    ...
    float const rtau = 1.0f / tau;
    float const rtau1 = 1.0f - rtau;

    for (int j = 0; j < nj; j++)
    {
        for (int i = 0; i < ni; i++)
        {
            int const i0 = I2D(ni, i, j);

            // Do the summations needed to evaluate the density and components of velocity
            float ro = f0[i0] + f1[i0] + f2[i0] + f3[i0] + f4[i0] + f5[i0] + f6[i0] + f7[i0] + f8[i0];
            ...
            const float f5eq = ro * faceq3
                               * (1.0f + 3.0f * (vx + vy) + 4.5f * (vx + vy) * (vx + vy) - v_sq_term);
            const float f6eq = ro * faceq3
                               * (1.0f + 3.0f * (-vx + vy) + 4.5f * (-vx + vy) * (-vx + vy)
                                  - v_sq_term);
            const float f7eq = ro * faceq3
                               * (1.0f + 3.0f * (-vx - vy) + 4.5f * (-vx - vy) * (-vx - vy)
                                  - v_sq_term);
            const float f8eq = ro * faceq3
                               * (1.0f + 3.0f * (vx - vy) + 4.5f * (vx - vy) * (vx - vy) - v_sq_term);
...
inline void solid_BC()
{
    for (int j = 0; j < nj; j++)
    {
        for (int i = 0; i < ni; i++)
        {
            ...
            if (solid[i0] == 0)
            {
                std::swap(f1[i0], f3[i0]);
                std::swap(f2[i0], f4[i0]);
                std::swap(f5[i0], f7[i0]);
                std::swap(f6[i0], f8[i0]);
            }
        }
    }
}
...
// and vice-verse
inline void per_BC(void)
{
    for (int i = 0; i < ni; i++)
    {
        auto const i0 = I2D(ni, i, 0);
        auto const i1 = I2D(ni, i, nj - 1);
...
inline void in_BC()
{
    float const vx_term = 1.f + 3.f * vxin + 3.f * vxin * vxin;
    float const f1new = roout * faceq2 * vx_term;
    float const f5new = roout * faceq3 * vx_term;
    float const f8new = f5new;

    for (int j = 0; j < nj; j++)
    {
        int const i0 = I2D(ni, 0, j);
        f1[i0] = f1new;
        f5[i0] = f5new;
        f8[i0] = f8new;
    }
